...
int main() {
   ...
   // ------------------ WRITE CODE HERE ------------------
    ...
    if (child_pid == -1) {
        perror("clone");
        printf("clone() system call failed..!!\n");
        exit(0);
    }
    ...
    */
    ...
    // got to know about the syscall() function from "man pidfd_open"
    // doing it here due to the above note "PID namespace of a process will only change the PID namespace of its subsequent children"
    setns(child_pidfd, CLONE_NEWPID);
    ...
    if (fork() == 0) {
        ...
        */
        ...
        setns(child_pidfd, CLONE_NEWUTS);
        // As we need to change the UTS namespace of the second child
