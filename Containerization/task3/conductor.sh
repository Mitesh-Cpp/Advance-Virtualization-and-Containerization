...
run()
{
    ...
    unshare --uts --pid --fork --net --mount --ipc --kill-child chroot "$CONTAINERDIR/$NAME/rootfs" /bin/bash -c "
    mount -t proc proc /proc
    mount -t sysfs sysfs /sys
    chmod 755 /
    exec $INIT_CMD_ARGS
    "
    # within the container, if I try to use ps, then it outputs: "Error, do this: mount -t proc proc /proc"
    # from this, I got to know the syntax of mount -t proc proc /proc
    # I added "mount -t sysfs sysfs /sys", as it was told in the below comments, but without this line also, I am getting correct expected working/output of the container as per the given sample/example image in the assignment document.
    # got to know that the corresponding directory inside the terminal is sys and not sysfs after running lscpu inside the container
    # using "--kill-child" flag reference: "https://man7.org/linux/man-pages/man1/unshare.1.html"
    # used "exec" to make sure that $INIT_CMD_ARGS is the entry program of the container. As without exec, it would be one more program in the container but it would not be the entry program.
...
exec()
{
    ...
    nsenter --target="$CONTAINER_INIT_PID" --mount --uts --ipc --pid --net -w -r sh -c "exec $EXEC_CMD_ARGS"
    # got to know about the flags from "man nsenter"
}
...
addnetwork()
{
    ...
    # found above syntax at "https://man7.org/linux/man-pages/man8/ip-link.8.html#EXAMPLES"
    ...
    # found above syntax under the heading "ip link set - change device attributes" at "https://man7.org/linux/man-pages/man8/ip-link.8.html#EXAMPLES"
