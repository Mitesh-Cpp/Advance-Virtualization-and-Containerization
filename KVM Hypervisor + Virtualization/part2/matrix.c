...
// for maintaining count, so that we can switch between vm1 and vm2 correctly
static int count;
...
void kvm_run_vm(struct vm *vm1, struct vm *vm2)
{
    ...
    while (1)
    {
        printf("VMFD: %d started running\n", vm_id[turn]->vm_fd);
        count ++;
        // If both vms, in total have run FRAC_A + FRAC_B times, then reset the count
        if(count == FRAC_A + FRAC_B)
            count = 0;
        ...
        if (ret < 0)
        {
            // in case the reason for KVM_RUN termination is INTR, set turn to 1 or 0 based on which vm should execute next depending on the given condition
            turn = (count >= FRAC_A) ? 1 : 0;
